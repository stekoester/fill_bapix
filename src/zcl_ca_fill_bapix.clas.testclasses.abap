CLASS ltcl_ca_fill_bapix DEFINITION FINAL FOR TESTING
  ...
  PRIVATE SECTION.
    ...
    METHODS s08_data_tab_datax_tab_as_incl FOR TESTING RAISING cx_static_check.
ENDCLASS.
...
CLASS ltcl_ca_fill_bapix IMPLEMENTATION.
  ...
  METHOD s07_data_str_datax_str_as_incl.
    ...
    DATA(bapi_datax) = VALUE ty_datax_as_incl( ).
    DATA(bapi_data) = VALUE ty_datax_as_incl( source_client = '100' ).
    DATA(expected_result) = VALUE ty_datax_as_incl( source_client = '100' ).
    TRY.
        zcl_ca_fill_bapix=>fill_bapix(
          EXPORTING
            bapi_data  = bapi_data
  ...
  METHOD s08_data_tab_datax_tab_as_incl.
    TYPES:
      BEGIN OF ty_data,
        client        TYPE mandt,
        source_client TYPE mandt,
    ...
    DATA(bapi_data) = VALUE tty_datax_as_incl( ( source_client = '100' ) ).
